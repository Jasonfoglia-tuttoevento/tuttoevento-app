...
export default function ChatPanel({ user }) {
  ...
  useEffect(() => {
    ...
    function handleOpenChat(event) {
      ...
      if (!detail?.participantUserId) {
        alert("Impossibile aprire la chat: participantUserId mancante.");
        return;
      }
  ...
  async function loadUsers() {
    const res = await fetch(`/api/chat/users?currentUserId=${user.id}`);
  ...
  async function createOrOpenConversation({
    ...
    if (!res.ok || !data.id) {
      alert("Errore apertura conversazione");
      return;
    }
            ...
            <aside className="bg-white border-r border-black/5 p-4 md:p-5 flex flex-col min-h-0">
              <div className="flex items-start justify-between gap-4 mb-5">
                ...
                <button
                  ...
                  className="w-10 h-10 rounded-2xl bg-[#f5f5f6] font-black shrink-0"
                >
                  ×
                </button>
              </div>
                  ...
                  conversations.map((conversation) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative w-full text-left rounded-3xl p-4 border transition ${
                          isActive
                            ? "bg-[#111] text-white border-[#111]"
                            : unreadCount > 0
                            ? "bg-[#fff7ed] text-black border-[#ff5a00]/30"
                            : "bg-[#f7f7f8] text-black border-black/5 hover:bg-black/[0.04]"
                        }`}
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {unreadCount > 0 && (
                            <span className="min-w-6 h-6 px-2 rounded-full bg-[#ff5a00] text-white text-[11px] flex items-center justify-center font-black">
                        ...
                        {conversation.bookingId && (
                          <p
                            className={`text-[11px] mt-3 font-black uppercase tracking-[2px] ${
                              isActive ? "text-white/35" : "text-[#ff5a00]"
                            }`}
                          >
                            Booking collegato
                          </p>
            ...
            <section className="hidden md:flex flex-col min-h-0">
              <div className="bg-white border-b border-black/5 p-5">
                ...
                {selectedConversation?.bookingId && (
                  <p className="text-sm text-[#ff5a00] font-black mt-2">
                    Chat collegata al booking #{selectedConversation.bookingId}
                  </p>
                )}
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3">
                {!selectedConversationId ? (
                  <div className="h-full flex items-center justify-center">
                    <p className="text-black/45 font-bold">
                      Avvia o seleziona una chat.
                    </p>
                  </div>
                ) : messages.length === 0 ? (
                  <div className="h-full flex items-center justify-center">
                  ...
                  messages.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {!isMine && (
                            <p className="text-xs font-black text-[#ff5a00] mb-1">
                          ...
                          <p
                            className={`text-[11px] mt-2 ${
                              isMine ? "text-white/45" : "text-black/35"
                            }`}
                          >
              ...
              <form
                ...
                className="bg-white border-t border-black/5 p-4 flex gap-3"
              >
                <input
                  value={message}
                ...
                <button
                  disabled={
                    !selectedConversationId || !message.trim() || isSending
                  }
