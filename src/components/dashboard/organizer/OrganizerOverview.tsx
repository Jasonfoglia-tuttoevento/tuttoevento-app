...
import { Card, INK, Inp, MUTED, O, SectionTitle } from "./shared";
...
export default function OrganizerOverview({ currentUser, bookings = [], plan }: OrganizerOverviewProps) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 12 }}>
        {[["Richieste inviate", bookings.length, false], ["Booking confermati", confirmed, false], ["Budget speso", fmt(spent), true]].map(([label, val, accent]) => (
          <div key={label as string} style={{ background: accent ? INK : "white", border: `1px solid ${accent ? "transparent" : "rgba(0,0,0,.06)"}`, borderRadius: 20, padding: "16px 18px" }}>
...
interface Artist {
  ...
  name?: string;
  eventTypes?: string[];
  publicPricing?: Record<string, Record<string, number>>;
  [key: string]: unknown;
}

function ContactRequestModal({ artist, onClose }: { artist: Artist; onClose: () => void }) {
  ...
  function onDurationChange(dk: string) {
    setDuration(dk);
