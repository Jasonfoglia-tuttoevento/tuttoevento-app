...
import { Card, INK, MUTED, O, ProBadge, ProLock, SectionTitle } from "./shared";
...
// ── Componente ──────────────────────────────────────────────────
export default function OrganizerCRM({ bookings, plan }: OrganizerCRMProps) {
  ...
  const withContract = bookings.filter((b) =>
    ["pending", "accepted", "confirmed"].includes(b.status || "")
  );

  const toReview = bookings.filter(
    (b) =>
      b.eventDate &&
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <Card>
        ...
        {crLoading ? (
          ...
          <p style={{ fontSize: 13, color: "rgba(0,0,0,.3)" }}>
            Nessuna richiesta di contatto inviata ancora. Vai su &quot;Trova artisti&quot; per contattare un artista.
          </p>
        ) : (
            ...
            {bookings.slice(0, 8).map((b, i) => (
              ...
              >
                <div style={{ minWidth: 0 }}>
                  ...
                  >
                    {b.artistName || "Artista"}
                  </p>
                  <p style={{ fontSize: 11, color: MUTED, margin: "2px 0 0" }}>
                    {b.eventDate || "—"} {b.eventTitle ? `· ${b.eventTitle}` : ""}
                  </p>
                </div>
                <span
                  style={{
            ...
            {withContract.map((b) => (
              <div key={b.id}>
                <button
                  type="button"
                  onClick={() =>
                    setExpandedContracts((prev) => ({ ...prev, [b.id]: !prev[b.id] }))
                  }
                  style={{
                    width: "100%",
                    display: "flex",
                ...
                >
                  <span style={{ fontSize: 13, fontWeight: 700, color: INK }}>
                    {b.artistName || "Artista"} — {b.eventDate || "—"}
                  </span>
                  <span style={{ fontSize: 12, color: O, fontWeight: 700 }}>
                    {expandedContracts[b.id] ? "Nascondi ▲" : "Vedi contratto ▼"}
                  </span>
                </button>
      ...
      <ProLock feature="L'export CSV dei dati" plan={plan}>
        <Card>
          <SectionTitle>
            Export dati <ProBadge />
          </SectionTitle>
          <p style={{ fontSize: 13, color: MUTED, marginBottom: 12 }}>
            Esporta tutto lo storico booking e contatti in formato CSV.
          </p>
          <button
            disabled
            style={{
