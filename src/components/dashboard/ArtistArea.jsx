  ...
  return (
    <div style={{ background:"rgba(255,90,0,.04)", border:"1px dashed rgba(255,90,0,.25)", borderRadius:18, padding:"20px", display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center", gap:12 }}>
      <div style={{ width:36, height:36, borderRadius:10, background:"#0a0a0b", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
        <svg width="14" height="16" viewBox="0 0 14 16" fill="none"><rect x="1" y="7" width="12" height="9" rx="2" fill="white"/><path d="M3.5 7V5C3.5 2.79 5.07 1 7 1C8.93 1 10.5 2.79 10.5 5V7" stroke="white" strokeWidth="1.8" strokeLinecap="round"/></svg>
      </div>
      <div>
        <p style={{ fontFamily:"Sora,sans-serif", fontWeight:800, fontSize:14, color:"#0a0a0b", margin:"0 0 4px" }}>Funzione Piano Pro</p>
      ...
      <div style={{ background:"rgba(255,90,0,.08)", border:"1px solid rgba(255,90,0,.2)", borderRadius:100, padding:"7px 16px", fontSize:11, color:"#ff5a00", fontWeight:700 }}>
...
function Tag({ label, active, onClick }) {
  return (
    <button type="button" onClick={onClick} style={{ padding:"5px 12px", borderRadius:100, fontSize:12, fontWeight:700, cursor:"pointer", border:`1px solid ${active?ORANGE:"rgba(0,0,0,.12)"}`, background:active?`${ORANGE}15`:"white", color:active?ORANGE:MUTED, fontFamily:"'Manrope',system-ui,sans-serif", transition:"all .15s" }}>
      {label}
    </button>
  );
}
...
/* ── Tab: Media Kit ─────────────────────────────────── */
// FIX: destructura le props direttamente invece di ricevere { props }
function TabMediaKit({
  ...
  const isPro = plan === "pro";
  // FIX: fallback sicuro per array
  const safeGenres = Array.isArray(musicGenres) ? musicGenres : [];
  const safeEvents = Array.isArray(eventTypes)  ? eventTypes  : [];

  function toggleGenre(g) {
    if (!isPro && safeGenres.length >= 1 && !safeGenres.includes(g)) return;
    setMusicGenres(prev => (Array.isArray(prev)?prev:[]).includes(g) ? prev.filter(x=>x!==g) : [...(Array.isArray(prev)?prev:[]), g]);
  }
  function toggleEvent(e) {
    setEventTypes(prev => (Array.isArray(prev)?prev:[]).includes(e) ? prev.filter(x=>x!==e) : [...(Array.isArray(prev)?prev:[]), e]);
  }
  ...
  return (
    <form onSubmit={saveArtistProfile} style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <Card>
        <SectionTitle>Bio{!isPro && <ProBadge />}</SectionTitle>
        <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
          <textarea value={bio||""} onChange={e=>setBio(e.target.value)} maxLength={isPro?undefined:150} rows={isPro?5:3}
          ...
          {!isPro && <p style={{ fontSize:11, color:MUTED }}>{(bio||"").length}/150 caratteri · <span style={{ color:ORANGE, fontWeight:700 }}>PRO: bio illimitata</span></p>}
        </div>
      </Card>

      <Card>
        <SectionTitle>Generi musicali{!isPro && <span style={{ fontSize:11, color:MUTED, fontWeight:400, marginLeft:8 }}>Max 1 (illimitati con <span style={{ color:ORANGE }}>PRO</span>)</span>}</SectionTitle>
        <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
          {MUSIC_GENRES.map(g=><Tag key={g} label={g} active={safeGenres.includes(g)} onClick={()=>toggleGenre(g)} />)}
        </div>
        {!isPro && safeGenres.length>=1 && (
          <div style={{ marginTop:10, background:"rgba(255,90,0,.06)", border:"1px solid rgba(255,90,0,.2)", borderRadius:12, padding:"8px 12px", display:"flex", alignItems:"center", gap:8 }}>
            <span>🔒</span><p style={{ fontSize:12, color:ORANGE, fontWeight:700, margin:0 }}>Con il piano Pro puoi selezionare generi multipli.</p>
          </div>
        )}
      </Card>
      ...
      <Card>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
          {EVENT_TYPES.map(e=><Tag key={e} label={e} active={safeEvents.includes(e)} onClick={()=>toggleEvent(e)} />)}
        </div>
      </Card>
      ...
      <Card>
        ...
        {photo && <img src={photo} alt="Preview" style={{ width:80, height:80, borderRadius:12, objectFit:"cover", marginTop:10, border:"1px solid rgba(0,0,0,.08)" }} />}
      </Card>

      <ProLock feature="Il video showcase" plan={plan}>
        <Card><SectionTitle>Video showcase <ProBadge /></SectionTitle><p style={{ fontSize:13, color:MUTED }}>Carica fino a 5 video per mostrare le tue performance.</p></Card>
      </ProLock>

      <Card>
        <SectionTitle>Link social{!isPro && <span style={{ fontSize:11, color:MUTED, fontWeight:400, marginLeft:8 }}>Max 1 (<span style={{ color:ORANGE }}>PRO</span>: illimitati)</span>}</SectionTitle>
      ...
      <ProLock feature="Il rider tecnico e press kit PDF" plan={plan}>
        <Card>
          <SectionTitle>Rider tecnico <ProBadge /></SectionTitle>
          <textarea value={rider||""} onChange={e=>setRider(e.target.value)} rows={4}
            placeholder="Es. 2 casse frontali, mixer 4 canali..."
      ...
      {artistMessage && <p style={{ fontSize:13, fontWeight:700, color:artistMessage.includes("Errore")?"#dc2626":"#16a34a" }}>{artistMessage}</p>}
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      ...
      <ProLock feature="La sincronizzazione con Google Calendar" plan={plan}>
        <Card>
          ...
          <button disabled style={{ background:"#0a0a0b", color:"white", border:"none", borderRadius:100, padding:"10px 20px", fontWeight:700, fontSize:13, cursor:"not-allowed", fontFamily:"'Manrope',system-ui,sans-serif", opacity:.5 }}>Connetti Google Calendar</button>
        </Card>
      </ProLock>
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))", gap:12 }}>
        {[["Booking totali",safe.length,false],["Confermati",confirmed.length,false],["Cachet incassato",fmt(revenue),true]].map(([label,val,accent])=>(
      ...
      <ProLock feature="Le analitiche avanzate del profilo" plan={plan}>
        <Card><SectionTitle>Analitiche avanzate <ProBadge /></SectionTitle><p style={{ fontSize:13, color:MUTED }}>Visite profilo, chi ha guardato e da dove — dati in tempo reale.</p></Card>
      </ProLock>
    </div>
  );
}
...
export default function ArtistArea(props) {
  ...
  return (
    <div id="artist-area" style={{ fontFamily:"'Manrope',system-ui,sans-serif", color:INK, display:"flex", flexDirection:"column", gap:16 }}>
      {/* FIX: passa le props direttamente invece di wrapping in { props } */}
      {tab==="mediakit"   && <TabMediaKit plan={plan} {...props} />}
