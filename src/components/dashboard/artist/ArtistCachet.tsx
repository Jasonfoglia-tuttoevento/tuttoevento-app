...
import { SCard, STitle, O, INK, MUTED, inp, DURATIONS } from "./shared";

interface ArtistCachetProps {
  pricing?: Record<string, Record<string, string | number>>;
  setPricing: React.Dispatch<React.SetStateAction<Record<string, Record<string, string | number>>>>;
  eventTypes?: string[];
  saveArtistProfile: (e: React.FormEvent) => Promise<void> | void;
  artistMessage?: string;
}

export default function ArtistCachet({ pricing = {}, setPricing, eventTypes = [], saveArtistProfile, artistMessage }: ArtistCachetProps) {
  ...
  function getPrice(ev: string, dur: string) { return (pricing?.[ev]?.[dur]) ?? ""; }
  function setPrice(ev: string, dur: string, val: string) {
