...
function BookingConfirmCard({ booking, onRefresh }: { booking: Booking; onRefresh?: () => void }) {
...
interface ArtistRichiesteProps {
  bookings?: Booking[];
  onRefreshBookings?: () => void;
}

export default function ArtistRichieste({ bookings = [], onRefreshBookings }: ArtistRichiesteProps) {
  const [requests, setRequests] = useState<ContactRequest[]>([]);
  const [loading, setLoading]   = useState(true);

  const STATUS_LABEL: Record<string, string> = { pending: "In attesa", contacted: "Contattato", connected: "Connesso", rejected: "Rifiutato" };
  const STATUS_COLOR: Record<string, string> = { pending: "#d97706", contacted: "#2563eb", connected: "#16a34a", rejected: "#dc2626" };
