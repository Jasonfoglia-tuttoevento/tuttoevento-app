...
import { SCard, STitle, ProBadge, ProLock, O, INK, MUTED, inp } from "./shared";
...
interface ArtistCalendarioProps {
  availableDates?: string[];
  setAvailableDates: React.Dispatch<React.SetStateAction<string[]>>;
  bookedSlots?: Array<string | BookedSlot>;
  plan?: string;
}

export default function ArtistCalendario({ availableDates = [], setAvailableDates, bookedSlots = [], plan }: ArtistCalendarioProps) {
  const [newDate, setNewDate] = useState("");
  const addDate = () => {
    if (!newDate || (availableDates || []).includes(newDate)) return;
    setAvailableDates(p => [...(p || []), newDate].sort()); setNewDate("");
  };
  const removeDate = (d: string) => setAvailableDates(p => (p || []).filter(x => x !== d));
      ...
      {slots.length > 0 && (
        <SCard>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {slots.map((slot, i) => {
              const s: BookedSlot = typeof slot === "string" ? { date: slot } : slot;
