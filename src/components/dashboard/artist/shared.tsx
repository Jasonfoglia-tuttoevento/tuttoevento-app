...
export const sel: React.CSSProperties = { ...inp, cursor: "pointer", appearance: "none", WebkitAppearance: "none" };
...
export function SCard({ children, style = {}, highlight = false }: { children?: React.ReactNode; style?: React.CSSProperties; highlight?: boolean }) {
  return (
    <div style={{
...
export function STitle({ children, sub }: { children?: React.ReactNode; sub?: React.ReactNode }) {
  return (
...
export function ProLock({ feature = "questa funzionalità", plan }: { feature?: string; plan?: string }) {
...
export function Inp({ label, value, onChange, placeholder, type = "text", disabled, hint }: { label?: string; value?: string | number; onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void; placeholder?: string; type?: string; disabled?: boolean; hint?: string }) {
  return (
...
export function SelectField({ label, value, onChange, options = [], placeholder = "" }: { label?: string; value?: string; onChange?: (e: React.ChangeEvent<HTMLSelectElement>) => void; options?: Array<string | { value: string; label: string }>; placeholder?: string }) {
  return (
    <Field label={label}>
...
export function KpiCard({ label, value, accent = false, orange = false }: { label?: string; value?: React.ReactNode; accent?: boolean; orange?: boolean }) {
  return (
    <div style={{
...
/* ── Componenti profilo ── */
export function PhotoUploader({ photo, onPhotoChange }: { photo?: string; onPhotoChange: (url: string) => void }) {
  ...
  async function handleFile(file?: File) {
    ...
    fd.append("file", file);
    try {
      const res = await fetch("/api/artists/upload-photo", { method: "POST", body: fd });
...
export function GenreMultiSelect({ selected = [], onToggle, isPro }: { selected?: string[]; onToggle: (g: string) => void; isPro?: boolean }) {
...
export function EventTypeMultiSelect({ selected = [], onToggle }: { selected?: string[]; onToggle: (e: string) => void }) {
  const [open, setOpen] = useState(false);
  const [rect, setRect]  = useState<DOMRect | null>(null);
