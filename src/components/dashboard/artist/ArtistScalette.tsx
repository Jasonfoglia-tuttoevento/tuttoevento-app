...
async function uploadTrackFile(file: File, userId: string, showId: number | string, trackId: number | string) {
...
function TrackCard({ track, idx, total, userId, showId, onUpdate, onRemove, onMove }: TrackCardProps) {
...
interface ArtistScaletteProps {
  bookings?: Booking[];
  currentUser?: { id?: string; [key: string]: unknown } | null;
}

/* ── Componente principale ── */
export default function ArtistScalette({ bookings = [], currentUser }: ArtistScaletteProps) {
  ...
  function createShow() {
    ...
    const show: Show = { id: Date.now(), title: newTitle.trim(), bookingId: null,
  ...
  function patchShow(id: number, patch: Partial<Show>) { persist(shows.map(s => s.id === id ? { ...s, ...patch } : s)); }

  function addTrack(showId: number) {
    const show = shows.find(s => s.id === showId);
    if (!show) return;
    const t: Track = { id: Date.now(), title: "", artist: "", bpm: "", key: "", duration: "", notes: "", audioUrl: "", fileName: "" };
    patchShow(showId, { tracks: [...(show.tracks || []), t] });
  }

  function patchTrack(showId: number, trackId: number, patch: Partial<Track>) {
    const show = shows.find(s => s.id === showId);
    if (!show) return;
    patchShow(showId, { tracks: show.tracks.map(t => t.id === trackId ? { ...t, ...patch } : t) });
  }

  function removeTrack(showId: number, trackId: number) {
    const show = shows.find(s => s.id === showId);
    if (!show) return;
    patchShow(showId, { tracks: show.tracks.filter(t => t.id !== trackId) });
  }

  function moveTrack(showId: number, idx: number, dir: number) {
    const show = shows.find(s => s.id === showId);
    if (!show) return;
