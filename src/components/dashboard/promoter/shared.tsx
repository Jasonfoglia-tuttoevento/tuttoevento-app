...
export function pct(n: number, d: number) { return d ? `${((n / d) * 100).toFixed(1)}%` : "0%"; }
...
export function STitle({ children }: { children?: React.ReactNode }) {
...
export function ProLock({ feature = "questa funzionalità", plan, children, collapsedLabel }: { feature?: string; plan?: string; children?: React.ReactNode; collapsedLabel?: string }) {
...
export function Inp({ label, value, onChange, placeholder, type = "text", disabled }: { label?: string; value?: string | number; onChange?: (e: React.ChangeEvent<HTMLInputElement>) => void; placeholder?: string; type?: string; disabled?: boolean }) {
  return (
...
export function KpiCard({ label, value, hint, accent, orange }: { label?: string; value?: React.ReactNode; hint?: string; accent?: boolean; orange?: boolean }) {
  return (
...
export function StatusBadge({ status }: { status?: string }) {
  const map: Record<string, { label: string; color: string }> = { pending: { label: "In attesa", color: "#d97706" }, reviewed: { label: "In revisione", color: "#2563eb" }, connected: { label: "Connessi", color: "#16a34a" }, rejected: { label: "Rifiutata", color: "#dc2626" } };
