  ...
  return (
    <>
      <style>{`
        ...
        .hh-content { position:relative; z-index:2; text-align:center; padding:120px 20px 100px; max-width:900px; width:100%; }
        ...
        .hh-badge p { font-size:.8rem; font-weight:700; color:rgba(255,255,255,.8); letter-spacing:.08em; text-transform:uppercase; font-family:'Manrope',sans-serif; margin:0; }
        @keyframes pulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.5;transform:scale(1.3)} }
        .hh-headline { font-family:'Sora',sans-serif; font-size:clamp(2.6rem,7vw,5.5rem); font-weight:900; line-height:1.05; letter-spacing:-.04em; color:#fff; white-space:pre-line; transition:opacity .4s,transform .4s; margin:0 0 24px; }
        .hh-headline.hidden { opacity:0; transform:translateY(10px); }
        .hh-headline em { color:#ff5a00; font-style:normal; }
        .hh-sub { font-family:'Manrope',sans-serif; font-size:clamp(.95rem,2.5vw,1.15rem); color:rgba(255,255,255,.55); margin:0 auto 40px; max-width:560px; line-height:1.7; }
        .hh-ctas { display:flex; gap:12px; justify-content:center; flex-wrap:wrap; }

        /* CTA buttons */
        .hh-cta-primary { background:#ff5a00; color:#fff; font-family:'Manrope',sans-serif; font-weight:800; font-size:1rem; padding:15px 28px; border-radius:100px; text-decoration:none; transition:all .25s; box-shadow:0 16px 40px rgba(255,90,0,.4); white-space:nowrap; }
        .hh-cta-primary:hover { background:#e85100; transform:translateY(-2px) scale(1.02); }
        .hh-cta-ghost { background:rgba(255,255,255,.06); color:#fff; font-family:'Manrope',sans-serif; font-weight:700; font-size:1rem; padding:15px 28px; border-radius:100px; text-decoration:none; border:1px solid rgba(255,255,255,.15); transition:all .25s; white-space:nowrap; }
        .hh-cta-ghost:hover { background:rgba(255,255,255,.12); }

        /* Mobile */
        @media(max-width:600px) {
          .hh-content { padding:100px 20px 80px; }
          .hh-ctas { flex-direction:column; align-items:center; gap:10px; }
          .hh-cta-primary, .hh-cta-ghost { width:100%; max-width:320px; text-align:center; padding:14px 20px; }
        }
      `}</style>
      ...
      <section className="hh-root">
        ...
        <div className="hh-content">
          <div className="hh-badge te-animate-up te-d1">
            ...
            <p>Marketplace eventi dal vivo · Italia</p>
          </div>
          <h1
          ...
          <div className="hh-ctas te-animate-up te-d4">
            ...
            <Link href="/register?role=promoter"  className="hh-cta-ghost">Sono un promoter</Link>
          </div>
        </div>
      </section>
    </>
